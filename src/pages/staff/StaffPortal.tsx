...
export default function StaffPortal() {
    ...
    const {data: staffQueue, isLoading: isQueueLoading, error: queueError} = useQuery({
    ...
    const {data: verifiedQueue, isLoading: isVerifiedLoading, error: verifiedError} = useQuery({
    ...
    const {data: swiftQueue, isLoading: isSwiftLoading, error: swiftError} = useQuery({
    ...
    const handle = (id: string, a: 'verify' | 'submit') => {
        setSelectedId(id);
        setAction(a);
        setOpen(true)
    }
        ...
        <Stack spacing={2}>
            ...
            <Paper sx={{p: 2}}>
                ...
                {staffQueue && (
                    <TableContainer>
                        <Table size="small"
                               sx={{'& tbody tr:nth-of-type(odd)': {backgroundColor: 'rgba(148,163,184,0.04)'}}}>
                            <TableHead>
                                <TableRow>
                            ...
                            <TableBody>
                                {!staffQueue?.payments || staffQueue.payments.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={7} align="center">No payments pending
                                    ...
                                    <TableRow key={p.id} hover>
                                        ...
                                        <TableCell align="center">
                                            <Button size="small" variant="contained"
                                                    onClick={() => handle(p.id, 'verify')}>Verify</Button>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        ...
                        <Table size="small"
                            ...
                            <TableBody>
                                {!verifiedQueue?.payments || verifiedQueue.payments.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={7} align="center">No verified payments awaiting SWIFT
                                            submission.</TableCell>
                                    </TableRow>
                                    ...
                                    <TableRow key={p.id} hover>
                                        ...
                                        <TableCell align="center">
                                            <Button size="small" variant="contained"
                                                    onClick={() => handle(p.id, 'submit')}>Submit to SWIFT</Button>
                                        </TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
